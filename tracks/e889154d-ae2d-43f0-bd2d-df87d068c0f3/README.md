...
транскрипция; её рабочий темп — `♩ = 171`. После A/B-сравнения предпоследняя
нота сокращена с целой, связанной с четвертью с точкой, до одной целой. Пауза
после финальной ноты сохраняет в MIDI исходное окно сравнения длиной `10,9`
секунды, но в PDF не печатается. Нотная сетка начинается с затакта `3/8`, после
которого следуют семь полных тактов `4/4`.
