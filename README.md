...
Recording MBID обозначает конкретную запись, а SHA-256 — конкретный файл.
Происхождение каждого аудиоартефакта хранится рядом с ним в sidecar-файле
`<имя>.provenance.json`. Формат sidecar описан корневой
[`artifact-provenance.schema.json`](artifact-provenance.schema.json): там
фиксируются размер, формат, длительность, SHA-256, источники, команда,
Git-коммит workflow и закреплённые инструменты. Это позволяет добавлять треки
без роста одного глобального provenance-файла.

Воспроизводимые действия оформляются как flake-команды. Например, референс и
фрагмент соло для «Blown Away» создаются так:
...
`flake.lock` в том же коммите закрепляет ревизию nixpkgs и, следовательно,
версии `yt-dlp`, FFmpeg и остальных инструментов. Каждая команда проверяет
ожидаемые хеши, поэтому незаметно получить другой исходник или результат не
получится.

Если внешний источник неизвестен — как у переданных вручную файлов «Волков
Океана» — это записывается явно; предполагаемый URL не выдумывается.

Проверить все sidecar-файлы и соответствующие локальные аудиофайлы:

```console
nix run .#verify-provenance
```
